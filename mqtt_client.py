...
class Config:
    ...
    def dump(self):
        print('host:{} port: {} user: {} passwd: {}'.format(self.host, self.port, self.user, self.passwd))
...
class App:
    ...
    def on_connect(client, userdata, flags, rc):
        instance = userdata
        print('Connection for client {} returned: {}'.format(instance.panel_id, rc))
        logging.debug('Connection for client {} returned: {}'.format(instance.panel_id, rc))
        return
    ...
    def on_message(client, userdata, message):
        instance = userdata
        print('client {} got message of topic{}: payload: {}'.format(instance.panel_id, message.topic, message.payload))
        logging.debug('client {} got message of topic{}: payload: {}'.format(instance.panel_id, message.topic, message.payload))
        return
    ...
    def on_open_vpn(client, userdata, message):
        instance = userdata
        logging.debug('client {} open_vpn payload: {}'.format(instance.panel_id, message.payload))
        #FIXME: set send logs flag
        return
    ...
    def __init__(self, panel_id, host, port, user, passwd):
        ...
        self.client = mqtt.Client(self.panel_id, clean_session = True, userdata = self)
        self.client.on_connect = App.on_connect
        self.client.on_message = App.on_message
        self.client.on_log = App.on_log
        # important message callbacks 
        self.client.message_callback_add('to_panel/' + self.panel_id + '/cmds/push_log', App.on_push_log)
        self.client.message_callback_add('to_panel/' + self.panel_id + '/cmds/open_vpn', App.on_open_vpn)
        self.client.username_pw_set(user, passwd)

        # the will message will be published on disceonnect from the broker
        self.client.will_set(topic = 'from_panel/' + self.panel_id + '/status', payload = 'offline', retain = True)

        self.client.connect(host, port)
        self.client.subscribe('to_panel/' + self.panel_id + '/#')

        # publish we online
        self.client.publish(topic = 'from_panel/' + self.panel_id + '/status', payload = 'online', retain = True)
        self.__quit = False
        return
